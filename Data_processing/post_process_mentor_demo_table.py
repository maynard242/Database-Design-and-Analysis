...
cur.execute('''CREATE TABLE IF NOT EXISTS mentor_demo_process AS SELECT * FROM mentor_demo;''')	
conn.commit()

cur.executre('''CREATE TABLE IF NOT EXISTS mentor_persona AS SELECT mentor_persona_id FROM match_history_process;''' )
conn.commit()
	
cur.execute('''SELECT * FROM mentor_demo_process JOIN mentor_persona ON mentor_demo_process.mentor_persona_id=mentor_persona.mentor_persona_id;''')
conn.commit()
...
cur.execute('''UPDATE mentor_demo_process SET current_career='Finance' WHERE CuFrrent_Career='Finance | Investor / Stockbroker';''')
conn.commit()
...
cur.execute('''UPDATE mentor_demo_process SET age_at_match=(match_history_process.match_start_date-mentor_demo_process.birthdate) WHERE mentor_demo_process.birthdate IS NOT NULL;''')
conn.commit

cur.execute('''UPDATE mentor_demo_process SET match_days="Not available' WHERE age_at_match IS NULL;''')
conn.commit
