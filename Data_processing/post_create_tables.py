...
ur.execute('''CREATE TABLE IF NOT EXISTS t1_mentor_platform_behavior AS SELECT mentor_persona_id, pair_id, lesson_id, canvas_word_count, time_on_canas_sub_1, user_first_sub FROM mentee_behavior FROM mentor_behavior;''')	
conn.commit()

cur.execute('''ALTER TABLE t1_mentor_platform_behavior ADD response_time INT;''')
conn.commit()

ur.execute('''ALTER TABLE t1_mentor_platform_behavior ADD by_mentor_word_count_zscore FLOAT;''')
conn.commit()
...
ur.execute('''CREATE TABLE IF NOT EXISTS t2_mentee_platform_behavior AS SELECT mentor_persona_id, pair_id, lesson_id, canvas_word_count, time_on_canas_sub_1, user_first_sub FROM mentee_behavior;''')	
conn.commit()

cur.execute('''ALTER TABLE t2_mentee_platform_behavior ADD PRIMARY KEY(mentor_persona_id);''')
conn.commit(

# Table t3_match_history - draws on match_history_process

ur.execute('''CREATE TABLE IF NOT EXISTS t3_match_history AS SELECT * FROM match_history_process;''')	
conn.commit()
...
cur.execute('''CREATE TABLE IF NOT EXISTS t4_demography AS SELECT * FROM mentor_demo_process;''')	
conn.commit()
...
cur.execute('''CREATE TABLE IF NOT EXISTS t5_imentor_master1516 AS SELECT mentor_persona_id, oop_status, same_eth_mentee, same_eth_mentee, mentor_eth, mentor_fgen, touchpoints, mentor_online_freq_1516, pair_online_freq, pair_meetings, pair_oop_mtgs_1516 FROM master_table_proces;''')	
conn.commit()
...
cur.execute('''CREATE TABLE IF NOT EXISTS t6_zipcodes AS SELECT Lives_in_Borough, Mailing_Zip_PostalCode FROM mentor_demo_process;''')	
conn.commit()
